...
export function InvitationCard({ sample, showBack = true, editorMode = false }) {
  return (
    <main className={`sample-page ${sample.className}`}>
      {showBack && <a className="sample-back" href="/">← Vow &amp; Vine</a>}
      {sample.className === 'olive' && <OliveGarden sample={sample} editorMode={editorMode} />}
    ...
    <main className="editor-page">
      ...
      <div className="editor-layout">
        <section className="editor-panel">
          ...
          {draft.className === 'olive' && <div className="editor-envelope-preview"><p className="editor-group-title">Envelope preview</p><div className="editor-envelope-card"><span>{draft.envelopeMessage || 'The start of our forever'}</span><strong>{draft.envelopeTitle || draft.title}</strong><i>{draft.monogram}</i></div><small className="editor-help">Edit the envelope wording below and see it update here.</small></div>}
          ...
          {draft.className === 'olive' && <><label>Envelope message<input value={draft.envelopeMessage || ''} onChange={(event) => update('envelopeMessage', event.target.value)} /></label><label>Envelope title<input value={draft.envelopeTitle || ''} onChange={(event) => update('envelopeTitle', event.target.value)} /></label></>}
        ...
        <div className="editor-preview"><p className="preview-label">LIVE PREVIEW</p><InvitationCard sample={draft} showBack={false} editorMode /></div>
      </div>
    </main>
  );
}
