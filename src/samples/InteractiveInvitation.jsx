...
export default function InteractiveInvitation({ sample, variant = 'olive' }) {
  ...
  return (
    <div className={`microsite microsite-${variant}`}>
      ...
      <section className="microsite-section story-section-sample" id="story"><div><p className="sample-eyebrow">{isOlive ? 'Our little story' : 'How it began'}</p><h2>{isOlive ? <>From a hello<br /><em>to forever.</em></> : <>It was always<br /><em>you and me.</em></>}</h2><p>{isOlive ? 'We met on an ordinary afternoon and kept finding reasons to make it extraordinary. Through shared tables, long walks, and a thousand small joys, we found our way home to each other.' : 'Somewhere between a shared table and a long walk home, we found our favorite person. We cannot wait to celebrate with the people who made our story possible.'}</p><button className="sample-rsvp secondary" onClick={() => setGalleryOpen(true)}>View our story <span>→</span></button></div><div className="story-quote">{isOlive ? '“The most beautiful things grow slowly.”' : '“The best is<br />yet to come.”'}</div></section>
      ...
      <section className="gift-section"><p className="sample-eyebrow">{isOlive ? 'A note on gifts' : 'Your presence is enough'}</p><h2>{isOlive ? <>Come as you are.<br /><em>That is enough.</em></> : <>But if you’d like<br /><em>to gift a little...</em></>}</h2><button onClick={() => alert('Registry details can be connected here.')}>{isOlive ? 'Read our note →' : 'View registry →'}</button></section>
