...
function Storefront() {
  ...
  return (
    <div className="site-shell">
      ...
      {preview && <div className="modal-backdrop" onClick={() => setPreview(null)}><div className="preview-modal" onClick={(event) => event.stopPropagation()}><button className="close" onClick={() => setPreview(null)}>×</button><img src={preview.image} alt="" /><div><p className="eyebrow">{preview.type} invitation</p><h2>{preview.name}</h2><p>Fully editable template · starts at ${preview.price}</p><div className="preview-actions"><a className="button outline" href={preview.sample || '#'}>Open live sample <span>↗</span></a>{preview.sample && <a className="button dark" href={preview.sample.replace('#sample=', '#edit=')}>Customize this design <span>→</span></a>}</div></div></div></div>}
